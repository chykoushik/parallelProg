...
*/
{
  ...
  pe = 0.0;
  ki = 0.0;
#pragma omp parallel private(i,j,k,d,d2,rij) shared(nd, f, np, pos, vel)
#pragma omp for reduction(+:pe, ki)
  for ( k = 0; k < np; k++ )
  {
/*
...
*/
{
  ...
  rmass = 1.0 / mass;
#pragma omp parallel private(j, i) shared(np, nd, pos, vel, dt, acc, rmass)
  for ( j = 0; j < np; j++ )
  {
    for ( i = 0; i < nd; i++ )
    {
